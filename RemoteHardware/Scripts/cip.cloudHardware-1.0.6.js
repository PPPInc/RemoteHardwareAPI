/*
 *
 * Charge It Pro Cloud Hardware Library v1.0.6
 * https://www.chargeitpro.com/
 *
 * Copyright (C) Charge It Pro, Corporation. All rights reserved.
 * 
 * Change Log: 5/18/2018 - Adding re-connectivity logic.
 */
...
CIP = new (function ($) {
    ...
    self.closeBatch; //Call this function to close the batch for a device.
    ...
    self._connect = function (done, fail) {
        ...
        self.connection.disconnected(function () {
            ...
            if (self.connection.state !== 1) {
                setTimeout(function () {
                    if (self.connection.state === 4) console.log("Reconnection Attempt: " + reconnectCount++);
                    self.connection.start().done(function () {
                        console.log('Successfully Reconnected');
                        reconnectCount = 1;
                    });
                }, 60000); // Restart connection after 1 minute.
            }
        });
    }
    ...
    self.creditSaleFunction = function (deviceName, amount, uniqueTransRef, cashier, transactionRef, street, zip) {
        var csMessage = {
            Action: "Transaction", TestMode: self.isTestMode, Data: { TransactionType: "CreditSale", Amount: amount, DeviceName: deviceName, UniqueTransRef: uniqueTransRef, Cashier: cashier, TransactionRef: transactionRef, Street: street, Zip: zip }
        };
        self._doTransaction(csMessage);
    }

    self.creditReturnFunction = function (deviceName, amount, uniqueTransRef, cashier, transactionRef, street, zip) {
        var crMessage = {
            Action: "Transaction", TestMode: self.isTestMode, Data: { TransactionType: "CreditReturn", Amount: amount, DeviceName: deviceName, UniqueTransRef: uniqueTransRef, Cashier: cashier, TransactionRef: transactionRef, Street: street, Zip: zip }
        };
        self._doTransaction(crMessage);
    }

    self.creditAuthFunction = function (deviceName, amount, cashier, transactionRef, street, zip) {
        var caMessage = {
            Action: "Transaction", TestMode: self.isTestMode, Data: { TransactionType: "CreditAuth", Amount: amount, DeviceName: deviceName, Cashier: cashier, TransactionRef: transactionRef, Street: street, Zip: zip }
        };
    ...
    self.saveCreditCardFunction = function (deviceName, cashier, transactionRef, street, zip) {
        var sccMessage = {
            Action: "Transaction", TestMode: self.isTestMode, Data: { TransactionType: "CreditSaveCard", Amount: "0.05", DeviceName: deviceName, Cashier: cashier, TransactionRef: transactionRef, Street: street, Zip: zip }
        };
        self._doTransaction(sccMessage);
    }

    self.debitSaleFunction = function (deviceName, amount, cashier, transactionRef, street, zip) {
        var dsMessage = {
            Action: "Transaction", TestMode: self.isTestMode, Data: { TransactionType: "DebitSale", Amount: amount, DeviceName: deviceName, Cashier: cashier, TransactionRef: transactionRef, Street: street, Zip: zip }
        };
    ...
    self.closeBatch = function (deviceName) {
        var closeMessage = {
            Action: "Transaction", TestMode: self.isTestMode, Data: { TransactionType: "CloseBatch", DeviceName: deviceName, DisplayText: displayText, Cashier: cashier, TransactionRef: transactionRef }
        };
    }
